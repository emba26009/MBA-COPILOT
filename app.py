...
def local_score(q,text):
    concept,terms=detect_concept(q)
    ql=q.lower();tl=text.lower()
...
def _rank(q,items,limit):
    rows=[(local_score(q,norm(it.get('text') or it.get('content') or it.get('passage'))),it) for it in items]
...
def retrieve(q,subject=None,limit=10):
    # Semantic retrieval is optional. If the OpenAI account has no credits,
    # immediately fall back to the local/lexical index instead of failing the tutor.
    items=[]
    if db.enabled() and os.getenv('OPENAI_API_KEY'):
        try:
            ...
            items=db.search_vector(v,subject,limit*3)
        except Exception:
            items=[]
    if not items and db.enabled():
        try: items=db.search_lexical(q,subject,limit*4)
        except Exception: items=[]
    if not items and subject and db.enabled():
        # If the selected subject has no lexical hits, search all subjects and
        # rank locally. This prevents a brittle "no evidence" result.
        try: items=db.search_lexical(q,None,limit*6)
        except Exception: items=[]
    if not items: items=INDEX
    filtered=_filter_items(items,subject)
    ranked=_rank(q,filtered,limit)
    if not ranked and subject:
        # Final fallback: local index can contain subject metadata with slightly
        # different spelling/capitalization; search it without a hard subject gate.
        ranked=_rank(q,INDEX,limit)
    return ranked
...
def ref(item,n):
    doc=item.get('document') or item.get('source') or item.get('filename') or 'Course material'
    loc=item.get('locator') or item.get('page') or item.get('slide') or item.get('sheet') or ''
...
def ask_gpt(q,subject=None):
    ...
    try:
        from openai import OpenAI
        r=OpenAI(api_key=key).responses.create(model=os.getenv('MBA_COPILOT_MODEL','gpt-4.1-mini'),input=f'You are a general GPT assistant, not course-grounded. Subject: {subject or "MBA"}. Answer clearly. Question: {q}')
        return r.output_text,None
    except Exception as e:
        msg=str(e)
        if 'insufficient_quota' in msg or 'credit_balance_exhausted' in msg or '429' in msg:
            return None,'OpenAI API credits are exhausted. The course-grounded MBA Copilot can still work using indexed MBA material, but Ask GPT and AI synthesis require available API credits.'
        return None,f'GPT request failed: {msg}'

def heuristic_answer(q,refs,concept):
    core={
      'fixed cost':'A fixed cost does not change with activity within the relevant range.',
      'variable cost':'A variable cost changes with the level of activity.',
      'contribution':'Contribution equals selling price minus variable cost per unit.',
      'break-even':'Break-even is the activity level where contribution covers fixed costs and profit is zero.'
    }.get(concept,f"The supplied material contains evidence related to '{concept}'.")
...
def answer(q,subject=None,mode='Teach Me'):
    concept,_=detect_concept(q)
    items=retrieve(q,subject)
    refs=[ref(x,i) for i,x in enumerate(items)]
    if not refs:
        return {'answer':'I could not find sufficiently relevant evidence in the selected MBA material. No answer was generated from unrelated material. Try selecting the subject containing the topic or upload/index the relevant class material.','sources':[],'concept':concept,'grounded':False,'ai_unavailable':not bool(os.getenv('OPENAI_API_KEY'))}
    key=os.getenv('OPENAI_API_KEY')
    if key:
        try:
            from openai import OpenAI
            ...
            # Do not let an exhausted OpenAI balance destroy course-grounded mode.
            if not any(x in str(e) for x in ('insufficient_quota','credit_balance_exhausted','429')):
                pass
...
def upload(body,ctype,token):
    ...
    subject=fields.get('subject','Unassigned')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=='/api/ask-gpt':
            ...
            self.send_json({'error':err},400) if err else self.send_json({'answer':text,'ai':True,'mode':'Ask GPT','grounded':False});return
